...
const ArticlePage = () => {
    ...
    if (!article) {
        return (
            <div className="min-h-screen bg-black flex flex-col items-center justify-center text-white">
                ...
                <button onClick={() => navigate('/articles')} className="text-[#4e77fc] hover:underline">חזרה לבלוג</button>
            </div>
        );
    }
    ...
    return (
        <PageTransition>
            <article className="min-h-screen bg-black pt-32 md:pt-40 pb-20 px-6">
                <div className="max-w-3xl mx-auto flex flex-col">
                    {/* Breadcrumbs */}
                    <div className="flex items-center gap-2 text-gray-500 text-sm font-bold mb-8 self-end">
                        <span className="text-white">{article.title}</span>
                        <ChevronRight size={14} />
                        <button onClick={() => navigate('/articles')} className="hover:text-[#4e77fc] transition-colors">מאמרים</button>
                        <ChevronRight size={14} />
                        <button onClick={() => navigate('/')} className="hover:text-[#4e77fc] transition-colors">ראשי</button>
                    </div>

                    <div className="text-right mb-12">
                        <div className="inline-block bg-[#4e77fc]/10 text-[#4e77fc] text-xs font-bold px-4 py-1.5 rounded-full mb-6">
                            {article.tag}
                        </div>
                        <h1 className="text-4xl md:text-6xl font-black text-white leading-tight mb-6">
                            {article.title}
                        </h1>
                        <p className="text-gray-400 font-bold tracking-widest">{article.date}</p>
                    </div>

                    <div className="w-full h-[400px] bg-white/5 rounded-2xl overflow-hidden mb-12 relative">
                    ...
                    <div 
                        className="prose prose-invert prose-lg max-w-none text-right font-light text-gray-300
                                   prose-headings:text-white prose-headings:font-black prose-headings:italic
                                   prose-h2:text-3xl prose-h2:mt-12 prose-h2:mb-6 prose-h2:text-[#4e77fc]
                                   prose-h3:text-2xl prose-h3:mt-8 prose-h3:mb-4
                                   prose-p:leading-relaxed prose-p:mb-6 prose-a:text-[#4e77fc]
                                   prose-ul:list-disc prose-ul:pr-6 prose-li:mb-2 prose-strong:text-white"
                        dangerouslySetInnerHTML={{ __html: article.content }}
                    />
                    
                    {/* Action Card */}
                    <div className="mt-20 bg-gradient-to-r from-[#020202] to-[#0a0a0a] border border-[#4e77fc]/20 p-8 md:p-12 rounded-2xl text-center md:text-right flex flex-col md:flex-row items-center justify-between gap-8">
                        <div>
                            <h3 className="text-2xl font-black text-white italic mb-2">רוצים לקחת את המותג שלכם שלב אחד קדימה?</h3>
                            <p className="text-gray-400 font-light max-w-md">הצוות המשפטי והקריאייטיב שלנו זמין להתייעצות קצרה וממוקדת, ללא עלות.</p>
                        </div>
                        <button onClick={() => navigate('/contact')} className="bg-[#4e77fc] text-white px-8 py-4 font-black whitespace-nowrap hover:bg-white hover:text-black transition-colors w-full md:w-auto">
                            דברו איתנו עכשיו
                        </button>
                    </div>
                </div>
