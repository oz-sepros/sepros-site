...
const SocialCarousel = () => {
    ...
    return (
        <div className="relative w-full flex items-center justify-center py-6 md:py-10 mt-6 md:mt-10 overflow-hidden">
            ...
            <button onClick={handlePrev} className="absolute right-2 md:right-8 z-30 p-2 md:p-4 bg-black/60 text-white rounded-full hover:bg-[#4e77fc] transition-colors"><ChevronRight size={24} /></button>
                    ...
                    return (
                        <div key={i}
                            ...
                            className={`relative transition-all duration-500 ease-in-out cursor-pointer flex-shrink-0 aspect-[9/16] rounded-2xl overflow-hidden
                              ${isActive
                                    ? 'w-[180px] md:w-[260px] scale-100 opacity-100 z-20 shadow-[0_0_30px_rgba(78,119,252,0.3)] border-2 border-[#4e77fc]'
                                    : 'w-[140px] md:w-[180px] scale-90 opacity-40 blur-[1px] z-10 hover:opacity-70'}`}>
            ...
            {/* Right Arrow */}
            <button onClick={handleNext} className="absolute left-2 md:left-8 z-30 p-2 md:p-4 bg-black/60 text-white rounded-full hover:bg-[#4e77fc] transition-colors"><ChevronLeft size={24} /></button>
        </div>
    )
}
...
const DepartmentPortfolio = ({ category }) => {
    if (category === 'social') {
        return (
            <div className="mt-16 md:mt-24">
                <h4 className="text-white font-black text-2xl md:text-3xl mb-4 italic border-r-4 border-[#4e77fc] pr-4">הפקות וידאו למובייל</h4>
                <SocialCarousel />
            </div>
        );
    }
    ...
    if (category === 'ppc' || category === 'seo' || category === 'analytics') {
        ...
        return (
            <div className="mt-16 md:mt-24">
                <h4 className="text-white font-black text-2xl md:text-3xl mb-8 italic border-r-4 border-[#4e77fc] pr-4">נתונים שמדברים</h4>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {[
                        ...
                        <div key={i} className="bg-[#0a0a0a] border border-white/5 p-8 text-center">
                            <div className="text-5xl font-black text-[#4e77fc] mb-2 dir-ltr">{stat.val}</div>
                            <div className="text-gray-400 text-sm font-bold tracking-widest">{stat.label}</div>
                        </div>
                    ))}
                </div>
            </div>
    ...
    if (category === 'tech' || category === 'design') {
        return (
            <div className="mt-16 md:mt-24">
                <h4 className="text-white font-black text-2xl md:text-3xl mb-8 italic border-r-4 border-[#4e77fc] pr-4">{category === 'tech' ? 'פרויקטים נבחרים (Web)' : 'ספרי מותג ו-UI/UX'}</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {[1, 2].map(i => (
                        <div key={i} className="bg-[#0a0a0a] border border-white/5 p-4 group cursor-pointer relative overflow-hidden">
                            <div className="aspect-video bg-black overflow-hidden relative">
                                <img src={category === 'tech' ? `https://images.unsplash.com/photo-1498050108023-c5249f4df085?auto=format&fit=crop&q=80&w=800&sig=${i}` : `https://images.unsplash.com/photo-1561070791-2526d30994b5?auto=format&fit=crop&q=80&w=800&sig=${i}`} alt="Project" loading="lazy" className="w-full h-full object-cover opacity-70 group-hover:scale-105 transition-transform duration-700" />
                                <div className="absolute inset-0 bg-[#4e77fc]/80 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
...
const DepartmentDetail = () => {
    ...
    return (
        <Reveal className="min-h-screen bg-black pt-32 md:pt-40 pb-20 text-right">
            <div className="max-w-[1400px] mx-auto px-6">
                <button onClick={() => navigate('/')} className="group text-[#4e77fc] flex items-center gap-2 mb-8 font-black tracking-widest text-xs">
                    חזרה לדף הבית <ArrowUpLeft className="rotate-90 group-hover:translate-x-2 transition-transform" />
                </button>
                <div className="grid lg:grid-cols-2 gap-12 md:gap-20 items-start">
                    <div>
                        <h1 className="text-4xl md:text-5xl font-black italic uppercase mb-6 text-[#4e77fc]">{dept.title}</h1>
                        <p className="text-gray-300 text-lg md:text-xl font-light mb-8 leading-relaxed">{dept.long}</p>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {dept.services.map((s, i) => (
                                <div key={i} className="flex items-center gap-3 bg-white/5 p-4 md:p-5 border border-white/5">
                                    <CheckCircle2 className="text-[#4e77fc] shrink-0" size={18} />
                    ...
                    <div className="aspect-square md:aspect-[4/5] bg-[#0a0a0a] border border-white/5 flex items-center justify-center p-8">
                        <img src="/src/assets/logo.png" alt="Sepros Icon" className="w-32 h-32 object-contain opacity-20 grayscale" />
                    </div>
                </div>

                <DepartmentPortfolio category={id || 'ppc'} />

                <div className="mt-20 md:mt-24 p-8 md:p-12 bg-[#4e77fc] text-black flex flex-col md:flex-row justify-between items-center gap-6 text-center md:text-right">
                    <h4 className="text-2xl md:text-3xl font-black italic">מוכנים להתחיל לעבוד?</h4>
                    <button id="dept_contact_btn" onClick={() => navigate('/contact')} className="bg-black text-white px-8 py-4 font-black tracking-widest hover:bg-white hover:text-black transition-all">בואו נתחיל</button>
                </div>
            </div>
        </Reveal>
    );
