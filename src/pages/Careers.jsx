...
const Careers = () => {
    ...
    return (
        <>
            <Reveal className="min-h-[85vh] bg-black pt-48 md:pt-56 pb-32 text-right">
                <div className="max-w-[1000px] mx-auto px-6">
                    <h1 className="text-5xl md:text-7xl font-black italic uppercase mb-6 text-[#4e77fc]">קריירה בספרוס</h1>
                    ...
                    <div className="space-y-6">
                        {jobs.map((job, i) => (
                            <div key={i} className="bg-[#0a0a0a] border border-white/5 p-6 rounded-xl hover:border-[#4e77fc]/50 transition-all cursor-pointer group" onClick={() => setExpandedJob(expandedJob === i ? null : i)}>
                                <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                                    <div>
                                        <h3 className="text-xl md:text-2xl font-black text-white group-hover:text-[#4e77fc] transition-colors mb-2">{job.title}</h3>
                                        <span className="text-[#4e77fc] text-[10px] font-bold tracking-widest bg-[#4e77fc]/10 px-3 py-1 rounded-sm">{job.type}</span>
                                    </div>
                                    <div>
                                        {expandedJob === i ? <Minus className="text-[#4e77fc] shrink-0" /> : <Plus className="text-gray-500 shrink-0" />}
                                    </div>
                                </div>
                                ...
                                <div className={`overflow-hidden transition-all duration-500 ease-in-out ${expandedJob === i ? 'max-h-[500px] mt-6 opacity-100' : 'max-h-0 opacity-0'}`}>
                                    <p className="text-gray-400 text-sm leading-relaxed mb-4">{job.fullDesc}</p>
                                    <h5 className="text-white font-bold text-sm mb-2">דרישות סף:</h5>
                                    <ul className="list-disc pl-4 pr-6 text-gray-400 text-sm space-y-1 mb-6">
