                    ...
                    {data.map((item, i) => (
                        <div key={i} className="faq-item reveal active border-b border-gray-200">
                            <h3 className="text-lg md:text-xl font-bold">
                                <button aria-expanded={openIndex === i} aria-label={openIndex === i ? "סגור שאלה" : "פתח שאלה"} onClick={() => setOpenIndex(openIndex === i ? -1 : i)} className={`w-full py-5 flex justify-between items-center text-right group gap-6 md:gap-8 cursor-pointer transition-colors ${openIndex === i ? 'text-[#2f4ea1]' : 'text-gray-800 group-hover:text-[#2f4ea1]'}`}>
                                    <span>{item.q}</span>
